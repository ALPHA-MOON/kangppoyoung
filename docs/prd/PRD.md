...
| FR-5.3 | 각 항목에 선정 근거(서버 `reason`='실무자 검색 N회·조회 M회…')를 그대로 표시(대표 질문 questionExample은 OnboardingItem에 없어 미표시). | 백엔드 `[구현됨]`, 프론트 `[구현됨]` |
...
- 프론트 규칙: 진행률 = 완료 항목 수/전체*100%(빈 결과 0% 가드), done 상태는 **order 키로 `localStorage`(`onboarding:done`) 영속**(새로고침에도 유지, 기간 무관 공유). 선정 근거는 서버 `reason`을 그대로 표시(인라인 재조립 없음), 대표 질문(questionExample)은 표시하지 않음. 로딩/에러/빈 상태 처리.
